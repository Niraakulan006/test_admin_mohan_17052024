...
$invest_id = "";
if(isset($_REQUEST['view_invest_id'])) {
    $view_invest_id = $_REQUEST['view_invest_id'];
}
$from = "";
...
$invest_list = array();
$invest_list = $obj->getAllRecords($GLOBALS['invest_table'], 'invest_id', $view_invest_id);

$invest_number = ""; $invest_date = ""; $party_id = ""; $party_name = "";$narration = ""; $amounts = array(); $payment_mode_ids = array(); $payment_mode_names = array(); $bank_ids = array(); 
$bank_names = array(); $total_amount = 0; $deleted = 0; $company_id = ""; $party_type = "";
...
if(!empty($invest_list)) {
    foreach($invest_list as $data) {
        ...
        if(!empty($data['invest_number']) && $data['invest_number'] != $GLOBALS['null_value']) {
            $invest_number = $data['invest_number'];
        }
        if(!empty($data['invest_date']) && $data['invest_date'] != "0000-00-00") {
...
$party_list = array();
// $party_list = $obj->getTableRecords($GLOBALS[$party_type.'_table'], $party_type.'_id', $party_id, '');
...
if(!empty($party_name) && $party_name !=$GLOBALS['null_value']){
    if (!empty($lr_id) && $lr_id != $GLOBALS['null_value']){
        $pdf->SetX(20);
        $pdf->Cell(85,4,'Payment For '.$lr_number.',',0,1,'L');
    }
        $pdf->SetX(20);
...
//  else if (!empty($lr_id) && $lr_id != $GLOBALS['null_value']){
//     $pdf->Cell(85,4,'Payment For '.$lr_number.',',0,1,'L');
// }
$party_y = $pdf->GetY();
...
$pdf->SetX(110);
$pdf->Cell(40,6,'Invest No ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$invest_number,0,1,'L');
$pdf->SetTextColor(0,130,0);
$pdf->SetX(110);
$pdf->Cell(40,6,'Invest Date ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$invest_date,0,1,'L');
$bill_y = $pdf->GetY();
...
$pdf->Output($from, $invest_number.'.pdf');
?>