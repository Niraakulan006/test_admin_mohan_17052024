<?php
    ...
    $file_name = "Suspense Payment Report";
    include("rpt_header.php");

    $pdf->SetFont('Arial','B',9);
    $pdf->SetX(10);
    $pdf->Cell(190,7,'Suspense Payment Report '.$date_display ,1,1,'C',0);
    ...
    if (!empty($payment_list)) {
        ...
        foreach ($grouped as $bill_number => $info) { 
            ...
            if(!empty($info['bill_date']) && (!empty($bill_number))) {
                ...
                // $bill_number = $obj->encode_decode('decrypt', $bill_number);
            ...
            if(!empty($info['party_id']) && $info['party_id'] != $GLOBALS['null_value']) {
    ...
    $pdf_name = "Suspense Payment Report( ". $date_display." ).pdf";
