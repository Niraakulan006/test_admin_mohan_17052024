...
$return_id = "";
if(isset($_REQUEST['view_return_id'])) {
    $view_return_id = $_REQUEST['view_return_id'];
}
$from = "";
...
$return_list = array();
$return_list = $obj->getAllRecords($GLOBALS['return_table'], 'return_id', $view_return_id);

$return_number = ""; $return_date = ""; $party_id = ""; $party_name = "";$narration = ""; $amounts = array(); $payment_mode_ids = array(); $payment_mode_names = array(); $bank_ids = array(); 
...
if(!empty($return_list)) {
    foreach($return_list as $data) {
        ...
        if(!empty($data['return_number']) && $data['return_number'] != $GLOBALS['null_value']) {
            $return_number = $data['return_number'];
        }
        if(!empty($data['return_date']) && $data['return_date'] != "0000-00-00") {
...
$pdf->SetX(110);
$pdf->Cell(40,6,'Return No ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$return_number,0,1,'L');
$pdf->SetTextColor(0,130,0);
$pdf->SetX(110);
$pdf->Cell(40,6,'Return Date ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$return_date,0,1,'L');
$bill_y = $pdf->GetY();
...
$pdf->Output($from, $return_number.'.pdf');
?>