...
$suspense_voucher_id = "";
if(isset($_REQUEST['view_suspense_voucher_id'])) {
    $view_suspense_voucher_id = $_REQUEST['view_suspense_voucher_id'];
}
$from = "";
...
$suspense_voucher_list = array();
$suspense_voucher_list = $obj->getAllRecords($GLOBALS['suspense_voucher_table'], 'suspense_voucher_id', $view_suspense_voucher_id);

$suspense_voucher_number = ""; $suspense_voucher_date = ""; $suspense_party_id = ""; $suspense_party_name = "";$narration = ""; $amounts = array(); $payment_mode_ids = array(); $payment_mode_names = array(); $bank_ids = array(); 
$bank_names = array(); $total_amount = 0; $deleted = 0; $company_id = "";

if(!empty($suspense_voucher_list)) {
    foreach($suspense_voucher_list as $data) {
        if(!empty($data['bill_company_id']) && $data['bill_company_id'] != $GLOBALS['null_value']) {
            $company_id = $data['bill_company_id'];
        }
        if(!empty($data['suspense_voucher_number']) && $data['suspense_voucher_number'] != $GLOBALS['null_value']) {
            $suspense_voucher_number = $data['suspense_voucher_number'];
        }
        if(!empty($data['suspense_voucher_date']) && $data['suspense_voucher_date'] != "0000-00-00") {
            $suspense_voucher_date = date('d-m-Y', strtotime($data['suspense_voucher_date']));
        }
...
$pdf->SetTitle('Suspense Voucher');
$pdf->SetFont('Arial','B',10);
$pdf->SetY(5);
$pdf->Cell(0,5,'Suspense Voucher',0,1,'C');
$header_y = $pdf->GetY();
$pdf->SetY(10);
...
$pdf->SetX(110);
$pdf->Cell(40,6,'Suspense Voucher No ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$suspense_voucher_number,0,1,'L');
$pdf->SetTextColor(0,130,0);
$pdf->SetX(110);
$pdf->Cell(40,6,'Suspense Voucher Date ',0,0,'L');
$pdf->SetTextColor(0,0,0);
$pdf->Cell(3,6,':',0,0,'L');
$pdf->Cell(57,6,$suspense_voucher_date,0,1,'L');
$bill_y = $pdf->GetY();
...
$pdf->Output($from, $suspense_voucher_number.'.pdf');
?>