<?php 
    ...
    $organization_id = ""; $branch_id =""; $consignee_id = ""; $consignor_id =""; $bill_type ="";$from_date =""; $to_date ="";$godown_id =""; $show_bill ="";
    $login_staff_id = "";
    ...
    if(!empty($from_date) && !empty($to_date)) {
        ...
        if(!empty($to_date)) {
            $from_date = date("Y-m-d", strtotime("-5 day", strtotime($to_date)));
        }
    }
    ...
    if(isset($_POST['search_lr_number'])) {
        $search_lr_number = $_POST['search_lr_number'];
    }
    ...
    $total_lr_count = 0;
    $total_lr_count = $obj->getClearanceReportCount($bill_company_id,$organization_id, $from_branch_id, $to_branch_id,$consignee_id,$consignor_id,$bill_type,'',$from_date, $to_date,$godown_id, $search_lr_number);
    
    $page_number = ""; $page_limit = 100;
    if(isset($_POST['page_number'])) {
        $page_number = $_POST['page_number'];
    }

    $total_records_list = array();
    $total_records_list = $obj->getClearanceReportList($bill_company_id,$organization_id, $from_branch_id, $to_branch_id,$consignee_id, $consignor_id,$bill_type, '',$from_date, $to_date,$godown_id, $search_lr_number, $page_number, $page_limit, $total_lr_count);
?>
<!DOCTYPE html>
<html lang="en">
<head>
                    ...
                    <div class="card-header">
                        <div class="row">
                            <div class="col-lg-8 col-md-8 col-8">
                                <h5 class="text-white">Clearance Report</h5>
                            </div>
                        </div>
                    </div>
                       ...
                       <form name="report_form" method="post">
                            ...
                            <div class="row px-2 mt-3">
                                ...
                                <div class="col-lg-2 col-md-4 col-6">
                                    <div class="form-group mb-1">
                                        <div class="form-label-group in-border pb-2">
                                            <input type="text" name="search_lr_number"  class="form-control shadow-none" placeholder="" value="<?php if(!empty($search_lr_number)) { echo $search_lr_number; } ?>" onChange="Javascript:getReport();">
                                ...
                                <div class="col-sm-3 col-lg-2 form-group">
                                    <div class="row mx-0">
                                        <div class="col-6">
                                            <?php
                                                if(!empty($total_lr_count)) {
                                                    $page_count = 0;
                                                    $page_count = ceil($total_lr_count / $page_limit);
                                                    if(!empty($page_count)) {
                                            ?>
                                                        <select name="page_number" class="form-control" onchange="Javascript:getReport();">
                                        ...
                                        <div class="col-6 px-1">                                                    
                                            <?php if(empty($search_lr_number)) { ?>
                                            <button type="button" class="btn btn-success" type="button" onClick="open_order_report('<?php echo $from_branch_id; ?>','<?php echo $to_branch_id; ?>','<?php echo $from_date; ?>','<?php echo $to_date; ?>','<?php echo $bill_type; ?>','<?php echo $consignee_id; ?>','<?php echo $consignor_id; ?>', '<?php echo $page_number; ?>', '<?php echo $page_limit; ?>', '<?php echo $total_lr_count; ?>')"> <i class="fa fa-download"></i> Print </button>
                                            <?php } ?> 
                                        </div>
                                    </div>                                   
                                </div>
                            ...
                            <div class="table nowrap">
                                <div class="w-100 px-3 py-3">
                                    <div id="report_area" class="w-100">
                                        <table class="table nowrap" id="tbl_sales_list" style="border:1px solid black; width: 1200px;">
                                            <thead>
                                                <tr>
                                                <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 50px;">S.No</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">LR.No/Date</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 150px;">Consignor</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 150px;">Consignee</th>
                                                    ...
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">Qty/Weight</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">Amount</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">Bill type</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 150px;">Received Person</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">Received Mobile.No</th>
                                                    <th class="text-center px-2 py-2" style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px; width: 100px;">Received Identity</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <?php
                                                            ...
                                                            if(!empty($prefix)) { $index = $index + $prefix; } ?>
                                                                <tr>
                                                                    ...
                                                                    <td style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px;" class="text-center px-2 py-2">
                                                                        ...
                                                                        if($total_quantity !='' && !empty($total_weight)){
                                                                            echo $total_quantity.",".$total_weight;
                                                                        }
                                                                        else if($total_quantity!='' && $total_weight!=''){
                                                                            echo $total_quantity;
                                                                        }
                                                                    ...
                                                                    <td style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px;" class="text-center px-2 py-2"><?php if(!empty($data['total'])){ echo $data['total']; }?></td>
                                                                    ...
                                                                    <td style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px;" class="text-center px-2 py-2">
                                                                        <?php
                                                                            if(!empty($data['received_person']) && $data['received_person'] != $GLOBALS['null_value']) {
                                                                                $data['received_person'] = $obj->encode_decode('decrypt', $data['received_person']);
                                                                                echo $data['received_person'];
                                                                            }
                                                                        ?>
                                                                    </td>
                                                                    <td style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px;" class="text-center px-2 py-2">
                                                                        <?php
                                                                            if(!empty($data['received_mobile_number']) && $data['received_mobile_number'] != $GLOBALS['null_value']) {
                                                                                $data['received_mobile_number'] = $obj->encode_decode('decrypt', $data['received_mobile_number']);
                                                                                echo $data['received_mobile_number'];
                                                                            }
                                                                        ?>
                                                                    </td>
                                                                    <td style="border-left: 1px solid #000; border-top: 1px solid #000; border-bottom: 1px solid #000; border-right: 1px solid #000; text-align: center; padding: 2px 10px; font-size: 13px; vertical-align: middle; height: 30px;" class="text-center px-2 py-2">
                                                                        <?php
                                                                            if(!empty($data['received_identification']) && $data['received_identification'] != $GLOBALS['null_value']) {
                                                                                $data['received_identification'] = $obj->encode_decode('decrypt', $data['received_identification']);
                                                                                echo $data['received_identification'];
                                                                            }
                                                                        ?>
                                                                    </td>
                                                                </tr>
...
<script>
    $(document).ready(function(){
        ...
        $("#clearance_report").addClass("active");
...
<script type="text/javascript">
    ...
    function getToBranchFilter() {
        ...
        var post_url = "lr_bill_changes.php?get_to_branch_report_filter="+from_branch_id;
        jQuery.ajax({
...
<script>
     ...
     <script>
        function open_order_report(from_branch_id,to_branch_id,from_date,to_date,bill_type,consignee_id,consignor_id, page_number, page_limit, total_lr_count){
            var url = "reports/rpt_clearance_report.php?from_date="+ from_date +"&to_date="+ to_date +"&from_branch_id="+ from_branch_id +"&to_branch_id="+ to_branch_id +"&bill_type="+ bill_type +"&consignee_id="+ consignee_id +"&consignor_id="+ consignor_id+'&page_number='+page_number+'&page_limit='+page_limit+'&total_lr_count='+total_lr_count;
