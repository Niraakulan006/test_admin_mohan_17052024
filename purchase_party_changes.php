<?php
	...
	if(isset($_REQUEST['show_purchase_party_id'])) { 
        $show_purchase_party_id = $_REQUEST['show_purchase_party_id'];
        ...
        $country = "India";$state = "";$district = "";$city = "";$party_name = "";$mobile_number = "";$address = "";$email = "";$pincode = "";$identification = "";$party_type = 2;
        if(!empty($show_purchase_party_id)){
            $party_list = array();
            $party_list = $obj->getTableRecords($GLOBALS['party_table'],'party_id',$show_purchase_party_id);
            if(!empty($party_list)){
        ...
        $linked_purchase_party = 0;
        // if(!empty($show_purchase_party_id)){
        //      $linked_purchase_party = $obj->PaymentlinkedParty($show_purchase_party_id);
        // }

        ?>
        <form class="poppins pd-20 redirection_form" name="purchase_party_form" method="POST">
        <?php if(empty($add_custom_party)) { ?>
			<div class="card-header">
				<div class="row">
				   <?php if(empty($show_purchase_party_id)){ ?>
                        <div class="col-lg-8 col-md-8 col-8">
                            <h5 class="text-white">Add Purchase Party</h5>
                        </div>
                    <?php } else { ?>
                        <div class="col-lg-8 col-md-8 col-8">
                            <h5 class="text-white">Edit Purchase Party</h5>
                        </div>
                    <?php } ?>
					<div class="col-lg-4 col-md-4 col-4">
						<button class="btn btn-danger float-right" style="font-size:11px;" type="button" onclick="window.open('purchase_party.php','_self')"> <i class="fa fa-arrow-circle-o-left"></i> &ensp; Back </button>
					</div>
				</div>
			</div>
        <?php } ?>
            <div class="row p-3">
                <input type="hidden" name="edit_id" value="<?php if(!empty($show_purchase_party_id)) { echo $show_purchase_party_id; } ?>">  
                <div class="col-lg-4 col-md-4 col-12 d-none">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <select name="party_type" class="form-control shadow-none">
                                <option value="">Select</option>
                                <option value="1" <?php if($party_type == '1'){ ?>selected<?php } ?>>Purchase</option>
                                <option value="2" <?php if($party_type == '2'){ ?>selected<?php } ?>>Sales</option>
                                <option value="3" <?php if($party_type == '3'){ ?>selected<?php } ?>>Both</option>
                            </select>
                            <label>Purchase Party Type(*)</label>
                        </div>
                    </div>
                </div>  
                <div class="col-lg-4 col-md-4 col-12">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            ...
                            <label>Purchase Party Name(*)</label>
                        </div>
                ...
                <div class="col-lg-4 col-md-4 col-12">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <select name="district" class="select2 select2-danger" placeholder="Select District" style="width: 100%;" onchange="Javascript:getDistricts('purchase_party', this.value, '');">
                ...
                <div class="col-lg-4 col-md-4 col-12">
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <select name="city" class="select2 select2-danger" placeholder="Select City" style="width: 100%;" onchange="Javascript:getCities('purchase_party', '', this.value);">
								<option value="">Select</option>
							</select>
							<label>City</label>
                        </div> 
                    ...
                    <div class="form-group">
                        <div class="form-label-group in-border">
                            <div class="input-group">
                                <input type="text" id="opening_balance" name="opening_balance" class="form-control shadow-none" required  value="<?php if(!empty($opening_balance)){echo $opening_balance;} ?>" onfocus="Javascript:KeyboardControls(this,'number',15,1);" maxlength="15" <?php if(!empty($linked_purchase_party)){ ?> readonly <?php } ?>>
                                <label>Opening Balance</label>
                                <div class="input-group-append" style="width:40%!important;">
                                  <select name="opening_balance_type" class="select2 select2-danger" placeholder="Select Opening Balance Type" style="width: 100%;" onchange="Javascript:InputBoxColor(this,'select');"  <?php if(!empty($linked_purchase_party)){ ?> disabled <?php } ?>>
                ...
                <?php if(!empty($linked_purchase_party) && !empty($show_purchase_party_id)){ ?>
                    <input type="hidden" name="opening_balance_type" value="<?php if(!empty($opening_balance_type)){ echo $opening_balance_type; } ?>">
                    <?php 
                } ?>
                <div class="col-md-12 pt-3 text-center">
                    <button class="btn btn-dark submit_button" type="button" onClick="Javascript:SaveModalContent('purchase_party_form', 'purchase_party_changes.php', 'purchase_party.php');">
                        Submit
                    </button>
                </div>
            </div>
        ...
        <script type="text/javascript">                
            jQuery(document).ready(function(){
                ...
                getCountries('purchase_party','<?php if(!empty($country)) { echo $country; } ?>', '<?php if(!empty($state)) { echo $state; } ?>', '<?php if(!empty($district)) { echo $district; } ?>', '<?php if(!empty($city)) { echo $city; } ?>');
            });
        </script>
		<?php
    } 
    ...
    if(isset($_POST['party_name'])) {
		...
		$valid_party = ""; $form_name = "purchase_party_form";
        $party_type = ""; $party_type_error = "";
        ...
        if(isset($_POST['party_name'])){
            ...
            //     if(empty($party_type)){
            //         $party_type_error = "Select Purchase Party Type";
            //     }
		...
		$result = ""; $lower_case_name = "";$prev_purchase_party_id = ""; $party_error = "";	
		...
		if(empty($valid_party)) {
			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
             $name_mobile_city = ""; $party_details = "";
				...
				if(!empty($mobile_number)){
					$prev_mobile_id = $obj->getTableColumnValue($GLOBALS['party_table'],'mobile_number',$mobile_number,'party_id');
					if(!empty($prev_mobile_id)){
						$error_mobile = $obj->getTableColumnValue($GLOBALS['party_table'],'party_id',$prev_mobile_id,'party_name');
                ...
                $update_payment = 0;
				if(empty($edit_id)) {
					if(empty($prev_purchase_party_id)) {
						if(empty($prev_mobile_id)){
							$action = "";
							if(!empty($party_name)) {
							...
							$columns = array('created_date_time', 'creator', 'creator_name','bill_company_id', 'party_id', 'party_type','party_name','lower_case_name', 'mobile_number','address','identification','state','district','city','others_city','pincode','opening_balance','opening_balance_type','name_mobile_city','party_details','deleted');
							$values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'","'".$bill_company_id."'", "'".$null_value."'", "'".$party_type."'", "'".$party_name."'", "'".$lower_case_name."'","'".$mobile_number."'","'".$address."'","'".$identification."'","'".$state."'","'".$district."'","'".$city."'","'".$others_city."'","'".$pincode."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$name_mobile_city."'","'".$party_details."'","'0'");
							$party_insert_id = $obj->InsertSQL($GLOBALS['party_table'], $columns, $values, $action);				
							if(preg_match("/^\d+$/", $party_insert_id)) {
								$party_id = "";
								if($party_insert_id < 10) {
									$party_id = "PURCHASE_PARTY_".date("dmYhis")."_0".$party_insert_id;
								}
								else {
									$party_id = "PURCHASE_PARTY_".date("dmYhis")."_".$party_insert_id;
								}
								if(!empty($party_id)) {
								...
								$columns = array('party_id');
								$values = array("'".$party_id."'");
								$party_update_id = $obj->UpdateSQL($GLOBALS['party_table'], $party_insert_id, $columns, $values, '');
								if(preg_match("/^\d+$/", $party_update_id)) {
                                         $update_payment =1;
									$result = array('number' => '1', 'msg' => 'Purchase Party Successfully Created','party_id' => $party_id);
								}
								else {
					...
					if(empty($prev_purchase_party_id) || $prev_purchase_party_id == $edit_id) {
						if(empty($prev_mobile_id) || $prev_mobile_id == $edit_id){
							...
							if(preg_match("/^\d+$/", $getUniqueID)) {
								...
								$columns = array(); $values = array();			
								$columns = array('creator_name','party_type','party_name', 'lower_case_name', 'mobile_number','address','identification','state','district','city','others_city','pincode','opening_balance','opening_balance_type','name_mobile_city','party_details');
								$values = array( "'".$creator_name."'", "'".$party_type."'","'".$party_name."'", "'".$lower_case_name."'","'".$mobile_number."'","'".$address."'","'".$identification."'","'".$state."'","'".$district."'","'".$city."'","'".$others_city."'","'".$pincode."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$name_mobile_city."'","'".$party_details."'");
                ...
                if ($update_payment == 1) {
                    ...
                    if(!empty($opening_balance) && $opening_balance != $GLOBALS['null_value'] && !empty($opening_balance_type) && $opening_balance_type != $GLOBALS['null_value']){
                        ...
                        $update_balance ="";
                        $update_balance = $obj->UpdateBalance($party_id,$bill_number,$bill_date,$bill_type,$party_id,$party_name,$party_type,$payment_mode_id, $payment_mode_name, $bank_id, $bank_name, $credit,$debit,$balance_type,'NULL','');
                    }else{
                        $payment_unique_id = "";
        ...
        if(!empty($search_text)) {
			...
			if(!empty($total_records_list)) {
				foreach($total_records_list as $val) {
					if( (strpos(strtolower(html_entity_decode($obj->encode_decode('decrypt', $val['party_name']))), $search_text) !== false) || (strpos($obj->encode_decode('decrypt', $val['mobile_number']), $search_text) !== false) ) {
						$list[] = $val;
					}
				}
			}
                    ...
                    if(!empty($show_records_list)) {
                        foreach($show_records_list as $key => $list) {
                            ...
                            <tr>
                                ...
                                <td class="text-center" onclick="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['party_id'])) { echo $list['party_id']; } ?>');">
                                    <?php
                                ...
                                <td>
                                    ...
                                    if(empty($access_error)) { ?>
                                        <a class="pr-2" href="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['party_id'])) { echo $list['party_id']; } ?>');" ><i class="fa fa-pencil" title="edit"></i></a>
                                    ...
                                    if(empty($access_error)){ ?>
                                        <?php
                                        $linked_count = 0;
                                        $linked_count = $obj->GetPartyLinkedCount($list['party_id']);

                                        if(!empty($linked_count)){
                                            ?>
                                                <span title="This Purchase Party can't be deleted">
                                                    <a class="pr-2" onclick="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['party_id'])) { echo $list['party_id']; } ?>');" <?php if(!empty($linked_count)){ ?>style="pointer-events: none; cursor: default;"<?php } ?> > <i class="fa fa-trash <?php if(!empty($linked_count)){ ?>text-secondary<?php }else{ ?>text-dark<?php } ?>"></i></a>
                                                </span>

                                            <?php
                                        }else{
                                            ?>
                                                <a class="pr-2" onclick="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['party_id'])) { echo $list['party_id']; } ?>');"> <i class="fa fa-trash text-dark"></i></a>
                                            <?php
                                        }
                                     ?>
                                    <?php } ?>
    ...
    if(isset($_REQUEST['delete_purchase_party_id'])) {
        $delete_purchase_party_id = $_REQUEST['delete_purchase_party_id'];
        $msg = "";
        if(!empty($delete_purchase_party_id)) {	
            $party_unique_id = "";
            $party_unique_id = $obj->getTableColumnValue($GLOBALS['party_table'], 'party_id', $delete_purchase_party_id, 'id');
            if(preg_match("/^\d+$/", $party_unique_id)) {
               
                $action = "";
                if(!empty($party_name)) {
                    $action = "Purchase Party Deleted - ".$obj->encode_decode("decrypt",$party_name);
                }

                $party_list = array();

				// $party_list = $obj->checkparty($delete_purchase_party_id);
                ...
                // if($delete == 1){
                    $delete_id = $obj->DeletePayment($delete_purchase_party_id);	
					$columns = array(); $values = array();						
					$columns = array('deleted');
					$values = array("'1'");
