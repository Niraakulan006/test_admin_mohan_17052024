<?php
	...
	if(isset($_REQUEST['show_expense_id'])) {
        $show_expense_id = $_REQUEST['show_expense_id'];

        $add_expense = 0; $profit_loss_form_name = "";
        $add_expense = $_REQUEST['add_expense'];
        $add_expense = trim($add_expense);

        $profit_loss_form_name = $_REQUEST['form_name'];
        $profit_loss_form_name = trim($profit_loss_form_name);
        ...
        ?>
        <form class="poppins pd-20 redirection_form" name="expense_form" method="POST">
			<div class="card-header">
				<div class="row p-2">
					<div class="col-lg-8 col-md-8 col-8 align-self-center text-white">
						<?php if(!empty($show_expense_id)){ ?>
                            <div class="h5 text-dark">Edit Expense</div> <?php 
                        } 
                        else { ?>
                            <div class="h5 text-dark">Add Expense</div> <?php
                        } ?>
					</div>
					<div class="col-lg-4 col-md-4 col-4">
                        <?php if(empty($profit_loss_form_name)){ ?>
 						    <button class="btn btn-danger float-right" style="font-size:11px;" type="button" onclick="window.open('expense.php','_self')"> <i class="fa fa-arrow-circle-o-left"></i> &ensp; Back </button>
                        <?php } ?>
					</div>
				</div>
			</div>
            <div class="row p-3">
                <input type="hidden" name="edit_id" value="<?php if(!empty($show_expense_id)) { echo $show_expense_id; } ?>">
                <div class="col-lg-12">
                        ...
                        <div class="col-lg-3 col-md-3 col-12 party_display">
                            <div class="form-group pb-2">
                                <div class="form-label-group in-border mb-0">
                                    <select name="expense_category_id" class="select2 select2-danger smallfnt" data-dropdown-css-class="select2 select2-danger" onchange="Javascript:HideDetails('Party');">
                        ...
                        <div class="col-lg-8">
                            ...
                            <input type="hidden" name="payment_row_count" value="0">
                                <table class="table nowrap cursor smallfnt w-100 table-bordered  payment_row_table fw-bold">
                                    <thead class="bg-dark smallfnt text-white">
                                        <tr style="white-space:pre;">
                                            <th>#</th>
                                            <th style="width:400px;">Tax</th>
                    ...
                    <div class="col-md-12 py-3 text-center">
                        <?php if(empty($profit_loss_form_name)){ ?>
                            <button class="btn btn-dark" type="button" onClick="Javascript:SaveModalContent('expense_form', 'expense_changes.php', 'expense.php');">
                            Submit
                          </button>
                        <?php } else{ ?>
                            <button class="btn btn-dark" type="button" onClick="Javascript:SaveModalContent('expense_form', 'expense_changes.php', '');">     Submit  </button>
                        <?php } ?>
                    </div>
                </div>     
            </div>
            ...
            <script>
                jQuery('#ExpenseModal').on('shown.bs.modal', function () {
                    jQuery('form[name="expense_form"]').find('select').select2({
                        dropdownParent: jQuery('#ExpenseModal')
                    });
                });
            </script>
        </form>
    ...
    if(isset($_POST['edit_id'])) {
        $expense_date = ""; $expense_date_error = "";$payment_tax_type="";$payment_tax_type_error=""; $expense_category_id = ""; $expense_category_id_error = "";$party_type = "";$payment_mode_ids = array(); $bank_ids = array(); $bank_names = array(); $payment_mode_names = array(); $amount = array(); $total_amount = 0; $payment_error = ""; $expense_category_name = ""; $narration = ""; $narration_error = ""; $selected_payment_mode_id = "";  $selected_amount = ""; $selected_amount_error = ""; $selected_bank_id = ""; $selected_payment_mode_id = ""; $selected_payment_mode_id_error = ""; $expense_number = ""; $selected_payment_tax_type = "";
        $form_name = "expense_form"; $valid_expense = "";  $payment_tax_types = array(); $selected_payment_tax_type_error = ""; 
        ...
        if(isset($_POST['expense_date'])){
            ...
            $expense_date_error = $valid->valid_date($expense_date, 'Expense Date', 1);
        }
        ...
        if(!empty($narration_error)) {
            if(!empty($valid_expense)) {
                $valid_expense = $valid_expense." ".$valid->error_display($form_name, "narration", $narration_error, 'textarea');
            }
            else {
                $valid_expense = $valid->error_display($form_name, "narration", $narration_error, 'textarea');
            }
        }
        ...
        if(isset($_POST['selected_payment_tax_type'])) {
            $selected_payment_tax_type = $_POST['selected_payment_tax_type'];
        }
        ...
        if(!empty($payment_mode_ids)) {
            for($i=0; $i < count($payment_mode_ids); $i++) {
                ...
                $available_balance = 0; $type_display = "";
           ...
           if(count($payment_mode_ids) <= 0) {
                ...
                if(!empty($valid_expense)) {
                    $valid_expense = $valid_expense." ".$valid->error_display($form_name, "selected_payment_mode_id", $selected_payment_mode_id_error, 'select');
                }
                else {
                    $valid_expense = $valid->error_display($form_name, "selected_payment_mode_id", $selected_payment_mode_id_error, 'select');
                }
            }
            if(count($payment_tax_types) <= 0) {
                ...
                if(!empty($valid_expense)) {
                    $valid_expense = $valid_expense." ".$valid->error_display($form_name, "selected_tax_type", $selected_payment_tax_type_error, 'select');
                }
                else {
                    $valid_expense = $valid->error_display($form_name, "selected_tax_type", $selected_payment_tax_type_error, 'select');
                }
            }
            ...
            if(count($amount) == 0) {
                ...
                if(!empty($valid_expense)) {
                    $valid_expense = $valid_expense." ".$valid->error_display($form_name, "selected_amount", $selected_amount_error, 'text');
                }
                else {
                    $valid_expense = $valid->error_display($form_name, "selected_amount", $selected_amount_error, 'text');
                }
            }
        }
        ...
        if(empty($valid_expense) && empty($payment_error)) {
			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                $balance = 0;
					...
					//  $expense_number = $obj->encode_decode('encrypt',$expense_number);
                ...
                if(empty($edit_id)) {	
					...
					if(!empty($expense_category_name) && $expense_category_name != $GLOBALS['null_value']) {
						$action = "New Expense Created. Name - ".($obj->encode_decode('decrypt', $expense_category_name));
					}
					$null_value = $GLOBALS['null_value'];
					$columns = array('created_date_time', 'creator', 'creator_name','bill_company_id', 'expense_id', 'expense_number', 'expense_date', 'expense_category_id', 'expense_category_name','payment_tax_type', 'narration', 'amount', 'payment_mode_id', 'payment_mode_name', 'bank_id', 'bank_name','total_amount', 'deleted');
					$values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'","'".$bill_company_id."'", "'".$null_value."'", "'".$expense_number."'", "'".$expense_date."'", "'".$expense_category_id."'", "'".$expense_category_name."'", "'".$payment_tax_types."'", "'".$narration."'", "'".$amount."'", "'".$payment_mode_ids."'", "'".$payment_mode_names."'", "'".$bank_ids."'", "'".$bank_names."'", "'".$total_amount."'", "'0'");
                    ...
                    if(preg_match("/^\d+$/", $expense_insert_id)) {
                        ...
                        if($expense_insert_id < 10) {
                            $expense_id = "EXP_".date("dmYhis")."_0".$expense_insert_id;
                        }
                        else {
                            $expense_id = "EXP_".date("dmYhis")."_".$expense_insert_id;
                        }
                        if(!empty($expense_id)) {
                ...
                if(!empty($balance) && $balance == 1) {
                    ...
                    if(!empty($payment_mode_id)){
                        for($i = 0; $i < count($payment_mode_id); $i++) {
                            ...
                            $party_type = "Expense Category";
                            $party_id = $expense_id;

                            $update_balance ="";
                            $update_balance = $obj->UpdateBalance($bill_id,$bill_number,$bill_date,$bill_type,$party_id,$GLOBALS['null_value'],$party_type,$payment_mode_id[$i],$payment_mode_name[$i],$bank_id[$i],$bank_name[$i],$credit,$debit, $balance_type, $payment_tax_type[$i],'');
                        }
                    } 
                }
    ...
    if(isset($_POST['page_number'])) {
		...
		if(isset($_POST['filter_expense_category_id'])) {
            ...
            $filter_expense_category_id = trim($filter_expense_category_id);
		} 
		...
		$total_records_list = $obj->getExpenseList($from_date, $to_date, $show_bill, $filter_expense_category_id);
                    ...
                    if(!empty($show_records_list)) {
                        foreach($show_records_list as $key => $list) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if(!empty($list['expense_number'])) { echo $list['expense_number']." <br>".date("d-m-Y",strtotime($list['expense_date'])); } ?>
                                </td>
                                <!-- <td>
                                    <?php
                                ...
                                <td class="text-center px-2 py-2">
                                    ...
                                    ?>
                                    <a target="_blank" href="reports/rpt_expense_a5.php?view_expense_id=<?php if(!empty($list['expense_id'])) { echo $list['expense_id']; } ?>&party_type=<?php if(!empty($list['party_type'])) { echo $list['party_type']; } ?>&from="><i class="fa fa-print"></i> &ensp; </a>
                                    <a target="_blank" href="reports/rpt_expense_a5.php?view_expense_id=<?php if(!empty($list['expense_id'])) { echo $list['expense_id']; } ?>&from=D"><i class="fa fa-download"></i> &ensp; </a>
                                    <?php if(empty($list['deleted'])){ 
                                        if(empty($access_error)) { ?>
                                            <a href="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['expense_id'])) { echo $list['expense_id']; } ?>');"><i class="fa fa-trash"></i> &ensp; </a>
                                        <?php }
                                    } ?>
                                </td>
                            </tr>
	...
	if(isset($_REQUEST['delete_expense_id'])) {
		$delete_expense_id = $_REQUEST['delete_expense_id'];
		$msg = ""; $delete_id = "";
		if(!empty($delete_expense_id)) {	
            $delete_id = $obj->DeletePayment($delete_expense_id);
			$expense_unique_id = "";
			$expense_unique_id = $obj->getTableColumnValue($GLOBALS['expense_table'], 'expense_id', $delete_expense_id, 'id');
            $expense_number= $obj->getTableColumnValue($GLOBALS['expense_table'], 'expense_id', $delete_expense_id, 'expense_number');
            $action = "";
            if(!empty($expense_number)) {
                $action = "Expense Deleted. Number - ".$expense_number;
            }
