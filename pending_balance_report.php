<?php 
    ...
    $from_date = date('Y-m-d', strtotime('-30 days')); $to_date = date("Y-m-d"); $filter_purchase_party_id = ""; $filter_consignor_id = ""; $filter_consignee_id = ""; $filter_account_party_id = ""; $current_date = date("Y-m-d");
                            ...
                            <div class="card-header">
                                <div class="row">
                                    <div class="col-lg-8 col-md-8 col-8">
                                        <h5 class="text-white">Pending Balance Report</h5>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="row mx-0">
                                    <?php if(!empty($filter_purchase_party_id) || !empty($filter_consignor_id) || !empty($filter_consignee_id) || !empty($filter_account_party_id)) { ?>
                                    ...
                                    <?php } ?>
                                    <?php if(empty($filter_consignor_id) && empty($filter_consignee_id) && empty($filter_account_party_id)) { ?>
                                        <div class="col-lg-2 col-md-4 col-6 py-2">
                                            <div class="form-group">
                                                <div class="form-label-group in-border">
                                    ...
                                    <?php } ?>
                                    <?php if(empty($filter_purchase_party_id) && empty($filter_consignee_id) && empty($filter_account_party_id)) { ?>
                                        <div class="col-lg-2 col-md-4 col-6 py-2">
                                            <div class="form-group">
                                                <div class="form-label-group in-border">
                                    ...
                                    <?php } ?>
                                    <?php if(empty($filter_purchase_party_id) && empty($filter_consignor_id) && empty($filter_account_party_id)) { ?>
                                        <div class="col-lg-2 col-md-4 col-6 py-2">
                                            <div class="form-group">
                                                <div class="form-label-group in-border">
                                    ...
                                    <?php } ?>
                                    <?php if(empty($filter_purchase_party_id) && empty($filter_consignor_id) && empty($filter_consignee_id)) { ?>
                                        <div class="col-lg-2 col-md-4 col-6 py-2">
                                            <div class="form-group">
                                                <div class="form-label-group in-border">
                                    ...
                                    <?php if(!empty($total_records_list)) { ?>
                                        <div class="col-lg-2 col-md-4 col-12 ms-auto py-2 text-end">
                                            ...
                                            <button class="btn btn-primary rounded-circle p-2 mx-1" style="font-size:14px; width:38px; height: 38px;" title="Print PDF" type="button" onclick="window.open('reports/rpt_pending_balance_report.php?filter_from_date=<?php if(!empty($from_date)) { echo $from_date; } ?>&filter_to_date=<?php if(!empty($to_date)) { echo $to_date; } ?>&filter_purchase_party_id=<?php if(!empty($filter_purchase_party_id)) { echo $filter_purchase_party_id; } ?>&filter_consignee_id=<?php if(!empty($filter_consignee_id)) { echo $filter_consignee_id; } ?>&filter_consignor_id=<?php if(!empty($filter_consignor_id)) { echo $filter_consignor_id; } ?>&filter_account_party_id=<?php if(!empty($filter_account_party_id)) { echo $filter_account_party_id; } ?>')"><i class="fa fa-print"></i></button>
                                            <button class="btn btn-info rounded-circle p-2 mx-1" style="font-size:14px; width:38px; height: 38px;" title="Download PDF" type="button" onclick="window.open('reports/rpt_pending_balance_report.php?filter_from_date=<?php if(!empty($from_date)) { echo $from_date; } ?>&filter_to_date=<?php if(!empty($to_date)) { echo $to_date; } ?>&filter_purchase_party_id=<?php if(!empty($filter_purchase_party_id)) { echo $filter_purchase_party_id; } ?>&filter_consignee_id=<?php if(!empty($filter_consignee_id)) { echo $filter_consignee_id; } ?>&filter_consignor_id=<?php if(!empty($filter_consignor_id)) { echo $filter_consignor_id; } ?>&filter_account_party_id=<?php if(!empty($filter_account_party_id)) { echo $filter_account_party_id; } ?>&from=D')"><i class="fa fa-download"></i></button>
                                        </div>
                                    <?php } ?>
                                </div>
                                <div class="row mx-0 justify-content-center">    
                                    <div class="col-lg-12 px-0">
                                        <div class="table-responsive table-bordered">
                                            <?php if(!empty($filter_purchase_party_id) || !empty($filter_consignor_id) || !empty($filter_consignee_id) || !empty($filter_account_party_id)) { ?>
                                                <table class="table table-nowrap table-bordered nowrap text-center smallfnt" id="tbl_pending_balance_report_list">
                                                    <thead class="smallfnt">
                                                        <tr>
                                                            <th colspan="6" class="text-center" style="border: 1px solid #dee2e6;font-weight: bold; font-size: 18px;">
                                                                Pending Balance Report <?php if(!empty($from_date)){ echo " ( " .date('d-m-Y',strtotime($from_date )) ." to ". date('d-m-Y',strtotime($to_date )). " )"; }?>
                                                            </th>
                                                        </tr>
                                                        <div>
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            ...
                                                            if(!empty($opening_credit) || !empty($opening_debit) || !empty($total_records_list)) {
                                                                ...
                                                                <tr>
                                                                    <th class="text-end" colspan="4">Current Balance</th>
                                                            ...
                                                            if(!empty($total_records_list)) {
                                                                foreach($total_records_list as $data) { 
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <th onclick="Javascript:ShowPartyBalance('<?php if(!empty($data['party_id'])) { echo $data['party_id']; } ?>', '<?php if(!empty($data['party_type'])) { echo $data['party_type']; } ?>');" style="cursor:pointer!important;">
                                                                            <?php
                                                                                $party_name = "";
...
<script type="text/javascript">
    ...
    function ShowPartyBalance(party_id, party_type) {
        if(party_type == 'Purchase Party') {
            if(jQuery('select[name="filter_purchase_party_id"]').length > 0) {
                jQuery('select[name="filter_purchase_party_id"]').val(party_id);
            }
        }
        else if(party_type == 'Consignor') {
            if(jQuery('select[name="filter_consignor_id"]').length > 0) {
                jQuery('select[name="filter_consignor_id"]').val(party_id);
            }
        }
        else if(party_type == 'Consignee') {
            if(jQuery('select[name="filter_consignee_id"]').length > 0) {
                jQuery('select[name="filter_consignee_id"]').val(party_id);
            }
        }
        else if(party_type == 'Account Party') {
            if(jQuery('select[name="filter_account_party_id"]').length > 0) {
                jQuery('select[name="filter_account_party_id"]').val(party_id);
            }
        }
        getReport();
    }
