<?php
	...
	if(isset($_REQUEST['show_receipt_id'])) {
        ...
        $receipt_date = date("Y-m-d"); $receipt_number = "";
        ...
        $party_list = $lr_list = array();
        // $party_list = $obj->getLRPartyList();
        $lr_list = $obj->getTableRecords($GLOBALS['lr_table'], '', '');
			...
			<div class="card-header">
				<div class="row p-2">
					<div class="col-lg-8 col-md-8 col-8 align-self-center">
						 <?php if(!empty($show_receipt_id)) { ?>
    						<h5 class="text-white">Edit Receipt</h5>
                        <?php } else { ?>
                            <h5 class="text-white">Add Receipt</h5>
                        <?php } ?>
					</div>
					<div class="col-lg-4 col-md-4 col-4">
						<button class="btn btn-danger float-right" style="font-size:11px;" type="button" onclick="window.open('receipt.php','_self')"> <i class="fa fa-arrow-circle-o-left"></i> &ensp; Back </button>
					</div>
				</div>
			</div>
            <div class="row p-3">
                <input type="hidden" name="edit_id" value="<?php if(!empty($show_receipt_id)) { echo $show_receipt_id; } ?>">
                <input type="hidden" name="receipt_number" value="<?php if(!empty($receipt_number)) { echo $receipt_number; } ?>">
                ...
                <div class="col-lg-12">
                    <div class="row justify-content-center">
                        ...
                        <div class="col-lg-3 col-md-3 col-12">
                            <div class="form-group pb-2">
                                <div class="form-label-group in-border mb-0">
                                    <select name="selected_party_type" class="select2 select2-danger smallfnt" data-dropdown-css-class="select2 select2-danger" onchange="Javascript:SelectedPartyList();">
                                        <option value="">Select</option>
                                        <option value="Consignor">Consignor</option>
                                        <option value="Consignee">Consignee</option>
                                        <option value="account_party">Account Party</option>
                                    </select>
                        ...
                        <div class="col-lg-3 col-md-3 col-12 party_display">
                            <div class="form-group pb-2">
                                <div class="form-label-group in-border mb-0">
                                    <select name="party_id" class="select2 select2-danger smallfnt" data-dropdown-css-class="select2 select2-danger" onchange="Javascript:HideDetails('Party');LRNumbersList();">
                                        <option value="">Select</option>
                                ...
                                <a href="Javascript:ViewReceiptPartyDetails('party');" class="d-none details_element" style="font-size: 12px;font-weight: bold;">Click to view details</a> <br>
                                   <a href="Javascript:ViewReceiptPendingDetails();" class="d-none details_element" style="font-size: 12px;font-weight: bold;">Click to view Pending details</a>
                            </div>        
                        ...
                        <div class="col-lg-3 col-md-3 col-12">
                            <div class="form-group pb-2">
                                <div class="form-label-group in-border mb-0">
                                    <select name="lr_id" class="select2 select2-danger smallfnt" data-dropdown-css-class="select2 select2-danger" onchange="Javascript:GetLRBillAmount();">
                                        <option value="">Select</option>
                                        <?php /* if(!empty($lr_list)) {
                                            foreach($lr_list as $data) { ?>
                                                <option value="<?php if(!empty($data['lr_id'])) { echo $data['lr_id']; } ?>"> <?php
                                                    if(!empty($data['lr_number'])) {
                                                        // $data['lr_number'] = html_entity_decode($obj->encode_decode('decrypt', $data['lr_number']));
                                                        echo $data['lr_number'];
                                                    } ?>
                                                </option> <?php
                                            }
                                    ...
                                    <label>LR(*)</label> 
                                         <div id="lr_amount_display" class="mt-1 small" style ="font-weight:bold;font-size:16px;color:#3351ab;"> </div>
                                        <div id="previous_receipt_amount" class="mt-1 small" style ="font-weight:bold;font-size:16px;color:#3351ab;"> </div>
                    ...
                    <div class="col-md-12 py-3 text-center">
                        <button class="btn btn-dark" type="button" onClick="Javascript:SaveModalContent('receipt_form', 'receipt_changes.php', 'receipt.php');">
                            Submit
                        </button>
                    </div>
                </div>     
    ...
    if(isset($_POST['edit_id'])) {
        $receipt_date = ""; $receipt_date_error = "";$payment_tax_type="";$payment_tax_type_error=""; $party_id = ""; $party_id_error = "";$party_type = "";$payment_mode_ids = array(); $bank_ids = array(); $bank_names = array(); $payment_mode_names = array(); $amount = array(); $total_amount = 0; $payment_error = ""; $party_name = ""; $narration = ""; $narration_error = ""; $name_mobile_city = ""; $selected_payment_mode_id = "";  $selected_amount = ""; $selected_amount_error = ""; $selected_bank_id = ""; $selected_payment_mode_id = ""; $selected_payment_mode_id_error = ""; $selected_payment_tax_type = ""; $selected_payment_tax_type_error = "";
        $form_name = "receipt_form"; $valid_receipt = ""; $payment_tax_types = array();
        $lr_number = ""; $lr_id = "";	$receipt_number =""; $party_type_error = ""; $receipt_amount_error = "";
        ...
        //         $valid_receipt = $valid_receipt." ".$valid->error_display($form_name, "payment_tax_type", $payment_tax_type_error, 'select');
        //     }
        //     else {
        //         $valid_receipt = $valid->error_display($form_name, "payment_tax_type", $payment_tax_type_error, 'select');
        //     }
        // }
        ...
        if(isset($_POST['lr_id'])) {
			$lr_id = $_POST['lr_id'];
            $lr_id = trim($lr_id);
            $lr_id_error = $valid->common_validation($lr_id, 'lr', 'select');
		}
        if(!empty($lr_id_error)){
            if(!empty($valid_receipt)) {
                $valid_receipt = $valid_receipt." ".$valid->error_display($form_name, "lr_id", $lr_id_error, 'select');
            }
            else {
                $valid_receipt = $valid->error_display($form_name, "lr_id", $lr_id_error, 'select');
            }
        }

        if(isset($_POST['selected_party_type'])) {
			$party_type = $_POST['selected_party_type'];
            $party_type = trim($party_type);
            $party_type_error = $valid->common_validation($party_type, 'Party Type', 'select');
		}
        if(empty($party_type)){
            $party_type_error = "Select the Party Type";
        }
        if(!empty($party_type_error)){
            if(!empty($valid_receipt)) {
                $valid_receipt = $valid_receipt." ".$valid->error_display($form_name, "selected_party_type", $party_type_error, 'select');
            }
            else {
                $valid_receipt = $valid->error_display($form_name, "selected_party_type", $party_type_error, 'select');
            }
        }
           ...
           if(count($payment_mode_ids) <= 0) {
            ...
            if(count($payment_tax_types) <= 0) {
                ...
                if(!empty($valid_voucher)) {
                    $valid_voucher = $valid_voucher." ".$valid->error_display($form_name, "selected_tax_type", $selected_payment_tax_type_error, 'select');
                }
                else {
                    $valid_voucher = $valid->error_display($form_name, "selected_tax_type", $selected_payment_tax_type_error, 'select');
                }
            }
            ...
            $lr_amount = 0; $receipt_amounts = array(); $receipt_amt = 0;  $balance_amount = 0;
        if(!empty($lr_id)){
            ...
            $balance_amount = $lr_amount - $receipt_amt;
          
            if($total_amount > $balance_amount){
                $receipt_amount_error = "Bill Amount is greater than LR Amount";
            } 
        }
        if(empty($valid_receipt) && empty($payment_error) && empty($receipt_amount_error)) {
            $check_user_id_ip_address = 0; $mobile_number =""; $city =""; $decrypted_party_name = ""; $decrypted_mbl ="";
			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                $lowercase_party_type ="";
                if(!empty($party_id) && !empty($party_type)){
                    $lowercase_party_type = strtolower($party_type);
                    $party_name = $obj->getTableColumnValue($GLOBALS[$lowercase_party_type.'_table'], $lowercase_party_type.'_id', $party_id, 'name');
                    ...
                    $mobile_number = $obj->getTableColumnValue($GLOBALS[$lowercase_party_type.'_table'], $lowercase_party_type.'_id', $party_id, 'mobile_number');
                    ...
                    // $city = $obj->getTableColumnValue($GLOBALS[$lowercase_party_type.'_table'], $lowercase_party_type.'_id', $party_id, 'city');
                    // if(!empty($city) && $city != $GLOBALS['null_value']){
                    //     $name_mobile_city .= ' ('.$city. ')';
                    // }
                    if(!empty($name_mobile_city) && $name_mobile_city != $GLOBALS['null_value']){
                        $name_mobile_city = $obj->encode_decode('encrypt', $name_mobile_city);
                    }
                    // $lowercase_party_type = $obj->getTableColumnValue($GLOBALS['party_table'], 'party_id', $party_id, 'lowercase_party_type');
                } else {
                ...
                if(empty($party_type)){
					...
					$receipt_number = $obj->new_automate_number($GLOBALS['receipt_table'], 'receipt_number');
					//  $receipt_number = $obj->encode_decode('encrypt',$receipt_number);
                    if(empty($receipt_number)){
                        $receipt_number = $GLOBALS['null_value'];
                    }
                    $balance = 0;	
                    $lr_bill_type = ""; $branch_id =""; $from_branch_id = ""; $to_branch_id ="";
                     if(!empty($lr_number)){
                        $lr_bill_type = $obj->getTableColumnValue($GLOBALS['lr_table'], 'lr_id', $lr_id, 'bill_type');
                        $to_branch_id = $obj->getTableColumnValue($GLOBALS['lr_table'], 'lr_id', $lr_id, 'to_branch_id');
                        $from_branch_id = $obj->getTableColumnValue($GLOBALS['lr_table'], 'lr_id', $lr_id, 'from_branch_id');

                        if($lr_bill_type == 'ToPay' || $lr_bill_type == 'Account Party'){
                            $branch_id = $to_branch_id;
                        }else if($lr_bill_type == 'Paid'){
                ...
                if(empty($edit_id)) {	
					...
					$columns = array('created_date_time', 'creator', 'creator_name','bill_company_id', 'receipt_id', 'receipt_number', 'receipt_date', 'lr_id', 'lr_number', 'party_id', 'party_name','party_type','payment_tax_type', 'narration', 'amount', 'payment_mode_id', 'payment_mode_name', 'bank_id', 'bank_name','total_amount', 'deleted','name_mobile_city','branch_id');
					$values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'","'".$bill_company_id."'", "'".$null_value."'", "'".$receipt_number."'", "'".$receipt_date."'", "'".$lr_id."'", "'".$lr_number."'", "'".$party_id."'", "'".$party_name."'","'".$party_type."'", "'".$payment_tax_types."'", "'".$narration."'", "'".$amount."'", "'".$payment_mode_ids."'", "'".$payment_mode_names."'", "'".$bank_ids."'", "'".$bank_names."'", "'".$total_amount."'", "'0'","'".$name_mobile_city."'","'".$branch_id."'");
                    $receipt_insert_id = $obj->InsertSQL($GLOBALS['receipt_table'], $columns, $values, $action);						
                    if(preg_match("/^\d+$/", $receipt_insert_id)) {
                        $receipt_id = "";
                        if($receipt_insert_id < 10) {
                            $receipt_id = "RECEIPT_".date("dmYhis")."_0".$receipt_insert_id;
                        }
                        else {
                            $receipt_id = "RECEIPT_".date("dmYhis")."_".$receipt_insert_id;
                        }
                        if(!empty($receipt_id)) {
                            $receipt_id = $obj->encode_decode('encrypt', $receipt_id);
                        }
                        $columns = array(); $values = array();						
                        $columns = array('receipt_id');
                        $values = array("'".$receipt_id."'");
                        $receipt_update_id = $obj->UpdateSQL($GLOBALS['receipt_table'], $receipt_insert_id, $columns, $values, '');
                        if(preg_match("/^\d+$/", $receipt_update_id)) {		
                            $result = array('number' => '1', 'msg' => 'Receipt Successfully Created');					
                            $balance = 1;							
                        }
                        else {
                            $result = array('number' => '2', 'msg' => $receipt_update_id);
                        }
                    }
                ...
                if(!empty($balance) && $balance == 1) {
                    ...
                    // $party_type = $party_type == '1' ? "Purchase Party" : "NULL";
                    $balance_type = 'Credit'; 
                    ...
                    // $lr_from_branch_id = "";
                    // $lr_from_branch_id = $obj->getTableColumnValue($GLOBALS['lr_table'], 'lr_id', $lr_id, 'from_branch_id');
                    ...
                    if(!empty($payment_mode_id)){
                        for($i = 0; $i < count($payment_mode_id); $i++) {
                            ...
                            $update_balance ="";
                            $update_balance = $obj->UpdateBalance($bill_id,$bill_number,$bill_date,$bill_type,$party_id,$party_name,$party_type,$payment_mode_id[$i],$payment_mode_name[$i],$bank_id[$i],$bank_name[$i],$credit,$debit, $balance_type, $payment_tax_type[$i],$branch_id);
            ...
            else if(!empty($receipt_amount_error)) {
				$result = array('number' => '2', 'msg' => $receipt_amount_error);
			}
    ...
    if(isset($_POST['page_number'])) {
        ...
        $filter_lr_id = "";
		if(isset($_POST['filter_lr_id'])) {
			$filter_lr_id = $_POST['filter_lr_id'];
            $filter_lr_id = trim($filter_lr_id);
		} 
		...
		$total_records_list = $obj->getReceiptList($from_date, $to_date, $show_bill, $filter_party_id, $filter_lr_id, $login_branch_id);
                    ...
                    if(!empty($show_records_list)) {
                        foreach($show_records_list as $key => $list) {
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if(!empty($list['receipt_number']) && $list['receipt_number'] != $GLOBALS['null_value']) { echo $list['receipt_number']." <br>".date("d-m-Y",strtotime($list['receipt_date'])); } ?>
                                </td>
                                <!-- <td>
                                    <?php
                                        if(!empty($list['investment_type'])) {
                                            $investment_type = "";
                                            $investment_type = $list['investment_type'];
                                            if($investment_type == "1"){
                                                $investment_type = "Purchase Investment";
                                            } elseif ($investment_type == '2'){
                                                $investment_type = "Expense Investment";
                                            } 
                                ...
                                <td>
                                    <?php
                                        if(!empty($list['name_mobile_city']) && $list['name_mobile_city'] != $GLOBALS['null_value']) {
                                            ...
                                            if(!empty($list['party_name']) && $list['party_name'] != $GLOBALS['null_value']) {
                                                $party_name = "";
                                ...
                                <td class="text-center px-2 py-2">
                                    ...
                                    ?>
                                        ...
                                        <a target="_blank" href="reports/rpt_receipt_a5.php?view_receipt_id=<?php if(!empty($list['receipt_id'])) { echo $list['receipt_id']; } ?>&party_type=<?php if(!empty($list['party_type'])) { echo $list['party_type']; } ?>&from="><i class="fa fa-print"></i> &ensp;</a>
                                        <a target="_blank" href="reports/rpt_receipt_a5.php?view_receipt_id=<?php if(!empty($list['receipt_id'])) { echo $list['receipt_id']; } ?>&from=D"><i class="fa fa-download"></i> &ensp;</a>
                                            <?php 
                                               $allow_delete = 0;
                                                $allow_delete = $obj->CheckBalanceForReceiptRestriction($list['receipt_id']);

                                            if(empty($list['deleted']) && empty($allow_delete)){ 
                                                if(empty($access_error)) { ?>
                                                <a href="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($list['receipt_id'])) { echo $list['receipt_id']; } ?>');"><i class="fa fa-trash"></i> &ensp;</a>
                                                <?php }
                                            } ?>
                                        </ul>
                                    </div> 
