<?php
	...
	if(isset($_SESSION[$GLOBALS['site_name_user_prefix'].'_user_id']) && !empty($_SESSION[$GLOBALS['site_name_user_prefix'].'_user_id'])) {
		if(!empty($GLOBALS['user_type']) && $GLOBALS['user_type'] != $GLOBALS['admin_user_type']) {
			...
			$permission_module = $GLOBALS['consignor_module'];
		}
	}
	if(isset($_REQUEST['show_consignor_id'])) { 
        $show_consignor_id = $_REQUEST['show_consignor_id'];
 		$opening_balance = "";$opening_balance_type = ""; $cooly_values = array();
        $name = ""; $mobile_number = ""; $consignorname = ""; $password = ""; $address = ""; $city = ""; $district = ""; $state = "Tamil Nadu"; $gst_number = ""; $country = "India"; $unit_count = 0; $unit_ids = array(); $unit_names = array(); $price_values = array();
		$identification = "";
        if(!empty($show_consignor_id)) {
            $consignor_list = array();
			$consignor_list = $obj->getTableRecords($GLOBALS['consignor_table'], 'consignor_id', $show_consignor_id);
		...
		?>
        <form class="poppins pd-20" name="consignor_form" method="POST">
			<div class="card-header">
				<div class="row">
				   <?php if(empty($show_consignor_id)){ ?>

						<div class="col-lg-8 col-md-8 col-8">
							<h5 class="text-white">Add Consignor</h5>
						</div>
                    <?php } else { ?>
						<div class="col-lg-8 col-md-8 col-8">
							<h5 class="text-white">Edit Consignor</h5>
						</div>
					<?php } ?>
					<div class="col-lg-4 col-md-4 col-4">
						<button class="btn btn-danger float-right" style="font-size:11px;" type="button" onclick="window.open('consignor.php','_self')"> <i class="fa fa-arrow-circle-o-left"></i> &ensp; Back </button>
					</div>
				</div>
			</div>
			<div class="row p-3">
				<input type="hidden" name="edit_id" value="<?php if(!empty($show_consignor_id)) { echo $show_consignor_id; } ?>">
				<div class="col-lg-3 col-md-6 col-12 mb-3">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            <input type="text" id="name" name="name" class="form-control shadow-none" placeholder="Party Name" value="<?php if(!empty($name)){ echo $name; } ?>">
                ...
                <div class="col-lg-3 col-md-6 col-12 mb-3">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            <input type="text" id="mobile_number" name="mobile_number" class="form-control shadow-none" placeholder="Party Number" value="<?php if(!empty($mobile_number)){ echo $mobile_number; } ?>" onfocus="Javascript:KeyboardControls(this,'mobile_number');">
                ...
                <div class="col-lg-3 col-md-6 col-12 mb-3">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            <input type="text" id="identification" name="identification" class="form-control shadow-none" placeholder="Party Identification" value="<?php if(!empty($identification)){ echo $identification; } ?>">
                ...
                <div class="col-lg-3 col-md-6 col-12 mb-3 d-none">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            ...
                            <select name="state" class="form-control shadow-none" id="state" onchange="Javascript:getStates('consignor',this.value, '', '');">
				...
				<div class="col-lg-3 col-md-6 col-12 mb-3">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            <select name="district" class="form-control shadow-none" id="district" onchange="Javascript:getDistricts('consignor', this.value, '');">
				...
				<div class="col-lg-3 col-md-6 col-12 mb-3">
                    <div class="form-group mb-1">
                        <div class="form-label-group in-border pb-2">
                            <select name="city" class="form-control shadow-none" id="city" onchange="Javascript:getCities('consignor', '', this.value);">
				...
				<div class="col-lg-6 col-md-10 col-12">
					...
					<div class="table-responsive tableheight">
						...
						<table class="table table-nowrap table-bordered border nowrap smallfnt cursor text-center w-100 unit_price_table">
							...
							<tbody>
								<?php
									if(empty($show_consignor_id) || empty($unit_ids)) {
										?>
			...
			<div class="col-md-12 pt-3 text-center">
				<button class="btn btn-dark" type="button" onClick="Javascript:SaveModalContent('consignor_form', 'consignor_changes.php', 'consignor.php');">
					Submit
				</button>
			</div>
			...
			<script type="text/javascript">                
				jQuery(document).ready(function(){
					getCountries('consignor','<?php if(!empty($country)) { echo $country; } ?>', '<?php if(!empty($state)) { echo $state; } ?>', '<?php if(!empty($district)) { echo $district; } ?>', '<?php if(!empty($city)) { echo $city; } ?>');
				});
			</script>
    ...
    if(isset($_POST['name'])) {	
        $name = ""; $name_error = ""; $address = ""; $address_error = ""; $city = ""; $city_error = ""; $mobile_number = ""; $mobile_number_error = ""; 
		$district = ""; $district_error = ""; $state = ""; $state_error = ""; $gst_number = ""; $gst_number_error = ""; 
        $form_name = "consignor_form"; $valid_client = ""; $opening_balance_type = ""; $opening_balance_type_error = "";
       ...
       if(empty($name)){
			...
			for($i=0; $i < count($unit_ids); $i++) {
				...
				if(empty($valid_client)) {
					...
					if(empty($valid_client)) {
						...
						$unit_price_ids[] = $obj->getUnitPriceUniqueID('Consignor', $edit_id, $unit_ids[$i]);
					}
				}
			}
		}
		...
		if(empty($valid_client) && empty($client_error)) {
			...
			if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
				...
				$prev_consignor_id = "";
				if(!empty($mobile_number)) {
					$prev_consignor_id = $obj->getTableColumnValue($GLOBALS['consignor_table'], 'mobile_number', $mobile_number, 'consignor_id');
					if(!empty($prev_consignor_id)) {
				...
				$consignor_id = ""; $update = 0; $update_payment = 0;
				if(empty($edit_id)) {
					if(empty($prev_consignor_id)) {
						$action = "";
						if(!empty($name)) {
						...
						$columns = array('created_date_time', 'creator', 'creator_name', 'consignor_id', 'name', 'address', 'city', 'mobile_number', 'district', 'state', 'gst_number' ,'identification','unit_id','unit_name','price_value', 'others_city','opening_balance','opening_balance_type','cooly_value','deleted');
						$values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$name."'", "'".$address."'", "'".$city."'", "'".$mobile_number."'", "'".$district."'", "'".$state."'", "'".$gst_number."'","'".$identification."'","'".$unit_ids."'","'".$unit_names."'","'".$price_values."'", "'".$others_city."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$cooly_values."'","'0'");
					...
					if(empty($prev_consignor_id) || $prev_consignor_id == $edit_id) {
						$getUniqueID = "";
						$getUniqueID = $obj->getTableColumnValue($GLOBALS['consignor_table'], 'consignor_id', $edit_id, 'id');
						if(preg_match("/^\d+$/", $getUniqueID)) {
							...
							$columns = array(); $values = array();						
							$columns = array('creator_name', 'name', 'address', 'city', 'mobile_number', 'district', 'state', 'gst_number', 'identification', 'unit_id','unit_name','price_value','others_city','opening_balance','opening_balance_type','cooly_value');
							$values = array("'".$creator_name."'", "'".$name."'", "'".$address."'", "'".$city."'", "'".$mobile_number."'", "'".$district."'", "'".$state."'", "'".$gst_number."'", "'".$identification."'","'".$unit_ids."'","'".$unit_names."'","'".$price_values."'","'".$others_city."'","'".$opening_balance."'","'".$opening_balance_type."'","'".$cooly_values."'");
							
							$party_update_id = $obj->UpdateSQL($GLOBALS['consignor_table'], $getUniqueID, $columns, $values, $action);
							if(preg_match("/^\d+$/", $party_update_id)) {
								$update = 1;	 $update_payment =1;
								$consignor_id = $edit_id;
				  ...
				  if ($update_payment == 1) {
                    ...
                    if(!empty($opening_balance) && $opening_balance != $GLOBALS['null_value'] && !empty($opening_balance_type) && $opening_balance_type != $GLOBALS['null_value']){
                        ...
                        $update_balance ="";
                        $update_balance = $obj->UpdateBalance($consignor_id,$bill_number,$bill_date,$bill_type,$consignor_id,$party_name,$party_type,$payment_mode_id, $payment_mode_name, $bank_id, $bank_name, $credit,$debit,$balance_type,'NULL','');
                    }else{
                        $payment_unique_id = "";
                        $payment_unique_id = $obj->getPartyOpeningBalanceInPaymentExist($consignor_id,$bill_type);
				...
				if(!empty($update) && $update == '1' && !empty($consignor_id)) {
					if(!empty($unit_ids) && $unit_ids != $GLOBALS['null_value'] && !empty($price_values) && $price_values != $GLOBALS['null_value']  && !empty($cooly_values) && $cooly_values != $GLOBALS['null_value']) {
						...
						for($i=0; $i < count($unit_ids); $i++) {
							if(!empty($unit_ids[$i]) && !empty($price_values[$i])) {
								$unit_price_update = $obj->UpdateUnitPrice('Consignor', $consignor_id, $name, $unit_ids[$i], $price_values[$i], $cooly_values[$i]);
							}
						}
					}
				}
		...
		if(!empty($search_text)) {
			...
			if(!empty($total_records_list)) {
				foreach($total_records_list as $val) {
					if( (strpos(strtolower($obj->encode_decode('decrypt', $val['name'])), $search_text) !== false) || (strpos(strtolower($obj->encode_decode('decrypt', $val['identification'])), $search_text) !== false) ) {
						$list[] = $val;
					}
				}
			}
                ...
                <?php
                    if(!empty($show_records_list)) {
                        foreach($show_records_list as $key => $data) { 
                            ...
                            $consignor_id = '';
                            $consignor_id = $obj->getTableRecords($GLOBALS['receipt_table'], 'consignor_id', $data['consignor_id'], 'id');

                            $consignee_id = '';
                            $consignee_id = $obj->getTableRecords($GLOBALS['receipt_table'], 'consignee_id', $data['consignor_id'], 'id');

                            ?>
                            <tr>
                                <td onclick="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['consignor_id'])) { echo $data['consignor_id']; } ?>');"><?php echo $key + 1; ?></td>
                                <td onclick="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['consignor_id'])) { echo $data['consignor_id']; } ?>');">
                                    <div class="w-100">
                                        <?php
                                            if(!empty($data['name'])) {
                                ...
                                <td>
                                    <?php $access_error = "";
										...
										if(empty($access_error)) { ?>
                                        <a class="pr-2" href="#" onclick="Javascript:ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['consignor_id'])) { echo $data['consignor_id']; } ?>');"><i class="fa fa-pencil"></i></a>
                                    <?php } ?>
                                    <?php
                                    $access_error = "";
                                    if(!empty($login_staff_id)) {
                                    ...
                                    if(empty($access_error)) { 
										   $linked_count = 0;
										$linked_count = $obj->GetConsignorLinkedCount($data['consignor_id']); 
									...
									<?php }else{ ?>
                                        <a class="pr-2" href="Javascript:DeleteModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($data['consignor_id'])) { echo $data['consignor_id']; } ?>');"><i class="fa fa-trash"></i></a>
                                    <?php } 
									}?>
                                </td>
                            </tr>
	...
	if(isset($_REQUEST['delete_consignor_id'])) {
		$delete_consignor_id = $_REQUEST['delete_consignor_id'];
		$msg = "";
		if(!empty($delete_consignor_id)) {
			$consignor_unique_id = "";
			$consignor_unique_id = $obj->getTableColumnValue($GLOBALS['consignor_table'], 'consignor_id', $delete_consignor_id, 'id');

			// $primary_consignor = "";
			// $primary_consignor = $obj->getTableColumnValue($GLOBALS['consignor_table'], 'consignor_id', $delete_consignor_id, 'primary_consignor');
			if(!empty($consignor_unique_id)) {
				if(preg_match("/^\d+$/", $consignor_unique_id)) {
					$name = "";
					$name = $obj->getTableColumnValue($GLOBALS['consignor_table'], 'consignor_id', $delete_consignor_id, 'name');
				
					$action = "";
					if(!empty($name)) {
						$action = "Consignor Deleted. Name - ".$obj->encode_decode('decrypt', $name);
					}
  					$delete_id = $obj->DeletePayment($delete_consignor_id);	

					$columns = array(); $values = array();						
					$columns = array('deleted');
					$values = array("'1'");
					$msg = $obj->UpdateSQL($GLOBALS['consignor_table'], $consignor_unique_id, $columns, $values, $action);
				}
			}
			else {
